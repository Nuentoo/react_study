...
import { StyledRedButton, StyledGreenButton, StyledOrangeButton } from '../../atoms/StyledButton';
...
export default function Task({ created_at, done, id, title }: TodoSupabase) {
  ...
  const todo = useMemo(() => ({created_at, done, id, title}), [created_at, done, id, title]); // ん〜…
  ...
  const updateTodoTables = useCallback(async (type: 'PATCH' | 'DELETE', nextTodo: TodoSupabase | TodoSupabase['created_at']) => {
    try {
      setIsLoading(true);
    ...
    <>
      {isLoading ? (<TodoSkelton />
        ...
        <>
          ...
          <p className="col-start-2 row-start-2"><small>post: {formatDate(created_at)}</small></p>
        </>
      )}
    </>
  );
